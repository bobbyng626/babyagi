...
class LEG_Version11:
    ...
    required_fields = {
        ...
        "speed_pro_score": int,
        "cold_door_score": int,
        "fitness_ratings_score": int
    }

    @classmethod
    def full_prompt_generator(cls, df_current_prompt) -> (str, str):
    ...
    def create_current_race(cls, df_current_prompt) -> List[str]:
        ...
        for rows in df_current_prompt.itertuples():
            ...
            current_race_record = f"Horse {rows.horse_num}, win odds {rows.win_odds_category}, " \
                                  ...
                                  f"Training Performance {rows.fitness_ratings_score}, \n" \
                                  f"---\n"
...
3. "Investment velocity" indicates the instant confidence change of the horse by the betting amount before the race start. It includes 1 index, Overbought.
...
4. "Past Race Performance" is the performance of the horse-jockey-trainer from past races ranking. It separately describes the general past performance of the horse, training and jockey-trainer, so we conducted three indexes to evaluate the past performance of these horses, training and the jockey-trainer combination. These three indexes gives only reference to judge the past race performance of the Jockey-trainer, Horse and Training, their performance might changed due to more training recently, take this as past reference and adjust the ranking rather than directly decide their ranking.
...
The Horse Performance Index also includes 5 values 0/1/2/3/4, indicating the past performance result of the horses. Horse Performance 0 means the horse has fewer Top 4 rank occurance race before, and 4 means a high number of Top 4 occurrence.

The Training Performance Index also includes 5 values 0/1/2/3/4, indicating the past training performance result of the horses. Training Performance 0 means the horse has fewer Top 4 rank occurance race before, and 4 means a high number of Top 4 occurance.
...
---
[Step 3]: Get the Past Performance of the horse-jockey-trainer, consider as a past race reference for jockey-trainer, horse strength and training performance separately.
---
[Step 4]: Analyze Win Odds for the winning chance of the horse, and compare difference horses win odds to see the current race-winning chance of each horse.
...
<SAMPLE CURRENT RACE>
---
Horse B678, win odds 2, jockey MMM, trainer BBB, jockey tier 3, trainer tier 3, jockey rate 4, trainer rate 4, OB 3, JT Performance 4, Horse Performance 3, Training Performance 4
---
Horse C456, win odds 1, jockey EEE, trainer YYY, jockey tier 2, trainer tier 3, jockey rate 3, trainer rate 2, OB 4, JT Performance 2, Horse Performance 2, Training Performance 2
---
Horse A123, win odds 4, jockey QQQ, trainer WWW, jockey tier 1, trainer tier 3, jockey rate 2, trainer rate 1, OB 0, JT Performance 1, Horse Performance 2, Training Performance 1
<SAMPLE RANKING>
RANKING:
...
STEP2 For Investment Velocity, it has OB 4, showing there is extreme confidence in this combination and has a very good past winning record.
STEP3 Consider the past race performance, the JT Performance had a medium Top 4 rank occurrence in past races as the index is 2, and the Horse performance is 2 also indicates a medium record for the horse, the Training performance is 2 also indicates a medium record for the horse, it shows a medium performance on past records.
STEP4 For the win odds, it has win odds 1, it has a lot of confidence and is trusted by public that can win, indicating it has a relatively high investment in the win, with very high chance to win the race, I must put it into Top 4 ranking, so its rank will always in [1st, 2nd, 3rd, 4th]
---
2nd: Horse B678
...
STEP2 For Investment Velocity, it has OB 3, showing there is extreme confidence in this combination and has a quite good past Top 4 record.
STEP3 Consider the past race performance, the JT Performance had a very high Top 4 occurrence in past races as the index is 4, and the Horse performance is 3 indicating a quite good record for the horse, and the Training performance is 4 indicating a good record for the horse, it shows a good performance on past records.
STEP4 For the win odds, it has win odds 2, indicating it has a relatively high investment in the win, with quite high chance to win the race.
---
